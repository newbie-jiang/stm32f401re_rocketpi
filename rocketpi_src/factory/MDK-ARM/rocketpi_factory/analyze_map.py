...
def build_table(
    ...
    lines.append(
        f"...  remaining {len(sorted_entries) - count} objects account for {remaining} bytes "
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=25,
        type=int,
        help="How many entries to display in each table (default: 25)",
    )
    parser.add_argument(
        "--output",
    ...
    report = render_report(entries, max(1, args.top))
