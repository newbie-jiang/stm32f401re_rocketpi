...
static uint32_t s_active_dir_key = 0U;
...
static uint32_t joystick_keypad_get_dir_key(const adc_joystick_sample_t *sample);
static uint32_t joystick_keypad_translate(uint32_t dir_key);
...
static void joystick_keypad_read(lv_indev_t * indev_drv, lv_indev_data_t * data)
{
    ...
    uint32_t dir_key = joystick_keypad_get_dir_key(&sample);
    if (dir_key != 0U)
    {
        uint32_t translated = joystick_keypad_translate(dir_key);
        s_active_dir_key = dir_key;
        s_last_key = translated;
