...
static void esp8266_at_core_handle_line(esp8266_at_core_t *ctx, const char *line)
{
    ...
    const bool ignore_ok_waiting_prompt =
        (ctx->phase == ESP8266_AT_PHASE_WAITING_PROMPT
         && event.type == ESP8266_AT_EVENT_TYPE_OK);

    bool handled_terminal = false;
    if (event.type == ESP8266_AT_EVENT_TYPE_RESPONSE
        && esp8266_at_strcasecmp(event.prefix, "+MQTTPUB") == 0)
    {
        if (esp8266_at_strcasecmp(event.payload, "OK") == 0)
        {
            event.command                 = ctx->pending_command;
            ctx->awaiting_reply            = false;
            ctx->phase                     = ESP8266_AT_PHASE_IDLE;
        ...
        else if (esp8266_at_strcasecmp(event.payload, "FAIL") == 0)
        {
            event.command                 = ctx->pending_command;
            ctx->awaiting_reply            = false;
            ctx->phase                     = ESP8266_AT_PHASE_IDLE;
    ...
    if (!handled_terminal && !ignore_ok_waiting_prompt)
    {
        switch (event.type)
        {
