...
/* 将暂存的数据推送到带区，并等待直接内存访问（DMA）完成 */
static void ws_test_commit_and_wait(void)
{
    (void)ws2812b_show();
}

/* duration_ms == 0 表示无限循环 */
static bool ws_test_should_stop(uint32_t start_ms, uint32_t duration_ms)
{
    if (duration_ms == 0U) {
        return false;
    }
    return (k_uptime_get_32() - start_ms) >= duration_ms;
}
                        ...
                        uint32_t on_time_ms, uint32_t off_time_ms, uint32_t duration_ms)
{
    ...
    while (!ws_test_should_stop(start_ms, duration_ms)) {
        ws2812b_fill(red, green, blue);
        ws_test_commit_and_wait();
        k_msleep(on_time_ms);
        if (ws_test_should_stop(start_ms, duration_ms)) {
            break;
        }
                          ...
                          uint32_t step_delay_ms, uint32_t duration_ms)
{
    ...
    while (!ws_test_should_stop(start_ms, duration_ms)) {
        ...
        for (int level = 255; level >= 0; --level) {
            ws2812b_fill((uint8_t)((red   * (uint16_t)level) / 255U),
                         (uint8_t)((green * (uint16_t)level) / 255U),
                         (uint8_t)((blue  * (uint16_t)level) / 255U));
            ws_test_commit_and_wait();
            k_msleep(step_delay_ms);
            if (ws_test_should_stop(start_ms, duration_ms)) {
                return;
