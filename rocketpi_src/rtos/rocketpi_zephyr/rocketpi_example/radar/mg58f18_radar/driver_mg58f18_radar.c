...
static bool mg58f18_radar_command_matches(uint8_t expected, uint8_t received)
{
    return (received == expected) || (received == (uint8_t)(expected ^ 0x80U));
}
...
static mg58f18_radar_status_t mg58f18_radar_core_process_byte(mg58f18_radar_core_t *ctx, uint8_t byte)
{
    ...
    if (checksum_ok && tail_ok)
    {
        ...
        {
            if (mg58f18_radar_command_matches(ctx->pending_command, ctx->last_frame.command))
            {
                ctx->last_error = MG58F18_RADAR_STATUS_OK;
            }
            else
...
static mg58f18_radar_status_t mg58f18_radar_wait_for_reply(uint32_t timeout_ms)
{
    ...
    while (mg58f18_radar_core_is_awaiting_reply(&s_core))
    {
        ...
        /* Yield so the RX polling thread can run when async UART is unavailable. */
        k_msleep(1);
    }
                                                             ...
                                                             mg58f18_radar_frame_t *response)
{
    ...
    if (last != NULL && mg58f18_radar_command_matches(command, last->command))
    {
        if (response != NULL)
        {
            *response = *last;
                                                         ...
                                                         uint8_t data4)
{
    ...
    if (!mg58f18_radar_command_matches(command, frame.command)
        || frame.data2 != data2 || frame.data3 != data3 || frame.data4 != data4)
    {
        return MG58F18_RADAR_STATUS_FRAME_ERROR;
    }
